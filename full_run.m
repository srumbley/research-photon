...
save('results/full/tank2/common_vars');

for iter=1:10
    %% Simulate detections
    
    % generate randomized array parameters (quantum efficiency, dark counts, crosstalk probability, dead pixels)
    q = q_mean + randn(array_size)*0.05^2;
    d = d_mean + randn(array_size)*0.0004^2;
...
%     p_ct(:,array_size(2),array_size(1)) = 0;
    ...
    save(sprintf('results/full/tank2/%d_simulated_data',iter),'q','d','p_ct','deadpix','detections','signal_mask','ct_mask','noise_mask','k','t');
    ...
    % SPIRAL parameters
    tau = 2.5; ainit = 100; max_iter = 70;
    pen_type = 'tv'; noisetype = 'binomial';

    A = @(x) signal_level_im.*x; 
    AT = @(x) signal_level_im.*x; 
...
%     D_ML = t_censored_avg*time_res*c*0.5;
    ...
    % SPIRAL estimation
    tau = 2; ainit = 1; max_iter = 50; penalty = 'tv';
    AT = @(x) x; A = @(x) x;
    imag_out = SPIRALTAP(t_censored_avg,A,tau, ...
        'noisetype','gaussian', ...
        'penalty',penalty, ...
        'maxiter',max_iter,...
...
%     figure;hist(lct(noise_mask),100);
    ...
    % SPIRAL parameters
    tau = 3.5; ainit = 10; max_iter = 70;
    pen_type = 'tv'; noisetype = 'binomial';

    A = @(x) signal_level_im.*x; 
    AT = @(x) signal_level_im.*x; 

    I_MAP_ct_censored = SPIRALTAP_NEW(k_ct_censored,A,tau,N, ...
        'noisetype',noisetype, ... % approx for binomial
        'penalty',pen_type, ...
        'maxiter',max_iter,... % `120
        'Initialization',I_ML,...
...
%     rom_ct_censored(isnan(rom_ct_censored)) = 0;
    ...
    % SPIRAL estimation
    tau = 0.7; ainit = 1; max_iter = 50; penalty = 'tv';

    AT = @(x) x; A = @(x) x;
    imag_out = SPIRALTAP(t_ct_censored_avg,A,tau, ...
        'noisetype','gaussian', ...
        'penalty',penalty, ...
        'maxiter',max_iter,...
    ...
    save(sprintf('results/full/tank2/%d_results',iter),'I_MAP','I_MAP_ct_censored','D_MAP','D_MAP_ct_censored','d_censored','d_ct_censored');
    save(sprintf('results/full/tank2/%d_performance',iter),'I_psnr','I_ct_censored_psnr','D_rmse','D_ct_censored_rmse','I_offset','I_ct_censored_offset','I_offset_psnr','I_ct_censored_offset_psnr','signal_censored','noise_censored','ct_censored','signal_ct_censored','noise_ct_censored','ct_ct_censored');
end
