...
function [x, varargout] = SPIRALTAP_NEW(y, A, tau, thres_count, varargin)
...
if (rem(length(varargin),2)==1)
    ...
    for ii = 1:2:(length(varargin)-1)
        switch lower(varargin{ii})
            ...
            case 'showfigiter';         showfigiter         = varargin{ii+1};
            case 'savetlv';             savetlv             = varargin{ii+1};
        otherwise
...
Axprevious = Ax;
xprevious = x;
grad = computegrad(y,Ax,AT,noisetype,logepsilon,thres_count);
...
if saveobjective
    ...
    objective(iter) = computeobjective(thres_count,x,y,Ax,tau,noisetype,logepsilon,penalty,WT);
end
if savetlv
    tlvs = zeros(maxiter+1,1);
    tlvs(iter) = tlv(x,'l1');
    alphas = zeros(maxiter+1,1);
    alphas(iter) = alpha;
end
if savereconerror
        ...
        case 1 % Barzilai-Borwein choice of alpha
            if monotone 
                ...
                while (accept == 0)
                    ...
                    objective(iter + 1) = computeobjective(thres_count, ...
                        x,y,Ax,tau,...
                        noisetype,logepsilon,penalty,WT);
                    tlvs(iter+1) = tlv(x,'l1');
...
if savetlv
    figure;plot(tlvs);title('tlvs');
    figure;plot(objective-(2*tau*tlvs));title('likelihood term');
    figure;plot(alphas);title('alphas');
end
if savereconerror
...
% =========================
function grad = computegrad(y,Ax,AT,noisetype,logepsilon,thres_count)
...
%             grad = AT(1 - (y./(Ax + logepsilon)));
        ...
        case 'gaussian'
            ...
            grad(mask) = 0;
        case 'binomial'
            grad = AT((thres_count-y) - (y./(Ax + logepsilon)));
    end
end
...
% ==========================
function objective = computeobjective(thres_count,x,y,Ax,tau,noisetype,logepsilon,...
    penalty,varargin)
% Perhaps change to varargin 
...
%         objective = thres_count*sum(Ax(:)) - sum(precompute(:));
    case 'gaussian'
